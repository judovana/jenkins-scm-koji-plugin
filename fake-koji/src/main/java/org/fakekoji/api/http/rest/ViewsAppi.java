...
public class ViewsAppi {
    ...
    private List<JenkinsViewTemplateBuilder> getNestedViews(List<TaskVariant> taskVariants, List<Platform> allPlatforms, List<Task> allTasks, List<String> projects, List<VersionlessPlatform> versionlessPlatforms, List<JDKVersion> jdkVersions, List<String> osses, List<String> ossesVersioned, List<String> arches, Optional<List<String>> filterOutViewsAsap) throws IOException {
        ...
        for (String tab : new String[]{"projects", "tasks", "platforms", "jdkVersions", "variants"}) {
            ...
            if (tab.equals("tasks")) {
                for (String subTask : new String[]{"all", "jdkVersions", "projects"}) {
                    JenkinsViewTemplateBuilder.JenkinsViewTemplateBuilderFolder subTasksFolder = jvtbFactory.getJenkinsViewTemplateBuilderFolder(subTask);
                    projectFolder.addView(subTasksFolder);
                    List<String> jps;
                    if (subTask.equals("all")) {
                        jps = Arrays.asList("");
                    } else if (subTask.equals("jdkVersions")) {
                        jps = jdkVersions.stream().map(q -> "-"+q.getId()).collect(Collectors.toList());
                    } else if (subTask.equals("projects")) {
                        jps = projects.stream().map(q -> ".*-"+q).collect(Collectors.toList());
                    } else {
