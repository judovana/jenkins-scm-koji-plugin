...
* `data` stores build log for each architecture and pull/incoming logs in src
...
##### Upload
`scp ./dir/file user@host:path/nvra`

`scp ./dir/nvra user@host:path`

`scp ./dir/nvra user@host:path/differentNvra`

 The uploaded file will be stored in `user@host:/path/n/v/r/a/`.

##### Download:
`scp user@host:path/nvra ./dir/`

`scp user@host:path/nvra ./dir/differentNvra`
...
##### Upload
`scp ./dir/logFile user@host:path/nvra/log`

`scp ./dir/logFile user@host:path/nvra/log/newFile`

The `logFile` will be stored in `user@host:/path/n/v/r/data/logs/a/`
...
will upload nvras to `user@host:/path/n1/v1/r1/a1` and `user@host:/path/n2/v2/r2/a2`, respectively.

Having `logFile1` and `logFile2` in `./dir`, then

`scp -r ./dir user@host:/path/nvra/logs`

will upload both log files to `user@host:/path/n/v/r/data/logs/a/`.

Having `data1` and `data2` in `./dir`, then

`scp -r ./dir user@host:/path/nvra/data`

will upload both files to `user@host:/path/n/v/r/data/`
...
`scp -r user@host:/path/nvra/logs ./dir`

will download all logs for given nvra.

`scp -r user@host:/path/nvra/data/logs ./dir`

and

`scp -r user@host:/path/nvra/data ./dir`
...
### HTTP File Listing
###### Default port: 9849
Here you can browse Fake Koji's folder structure.
### HTTP View
###### Default port: 80
Lists all products, their projects and each project's last stable build. In project's details there is list of all builds.
...
![Jenkins plugin config example](./docs/images/jenkins-scm-koji-plugin-config-01.png)
