...
public class KojiListBuilds implements FilePath.FileCallable<Build> {
    ...
    public Build invoke(File workspace, VirtualChannel channel) throws IOException, InterruptedException {
        ...
        for (String url : config.getKojiTopUrls()) {
            BuildMatcher bm = new BuildMatcher(url);
    ...
    private class BuildMatcher {

        private final String currentURL;

        public BuildMatcher(String url) {
            this.currentURL = url;
        }

        private Optional<Build> getResult() {
        ...
        /**
         * Archives are stored under {@link Constants#rpms} key, together with
         * RPMs.
         * <p>
         * Name, Version and Release are not received with info about archive.
         * We need to get it from the build. Arch is taken from configuration
         * and is later used to compose filepath. Unlike with RPMs, filename is
         * received here so we can store it.
         */
